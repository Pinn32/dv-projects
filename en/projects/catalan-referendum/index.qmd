...
![Datatypes and Attributes](./img/fig1-datatypes-attributes.png){#fig-datatypes-attributes fig-alt="Datatypes and Attributes"}
...
# Appendix I: Attributes Description {#appendix-attributes}
...
# Appendix II: Validating Values Before Modifying Data Types {#appendix-data-types}

In the beginning, only missing data of certain subsets were dropped via `.dropna()` method (@fig-drop-na).

![Dropping NAs from certain columns only](./img/fig4-drop-na-from-col.png){#fig-drop-na fig-alt="Dropping NAs from certain columns only"}

However, errors occurred subsequently when modifying data types via `.astype()` method (@fig-astype-error).

![Error when using `.astype(int)`](./img/fig5-error-astype-int.png){#fig-astype-error fig-alt="Error when using .astype(int)"}
...
![Check non-numerical strings](./img/fig6-check-non-num-str.png){#fig-non-num-str fig-alt="Check non-numerical strings"}
...
# Appendix III: Modifying Seaborn Histograms' Legend {#appendix-legend}
...
![First Attempt](./img/fig7-first-attempt.png){#fig-first-attempt fig-alt="First Attempt"}
