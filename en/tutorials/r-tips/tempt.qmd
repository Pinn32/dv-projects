...
- **用 `nrow()` 得到行数, `pull()` 从 dataframe 中取出单个值**
- **善用 stringr 和 regex 处理文本**
    - 字符个数: `mutate(text_length = str_length(text))`
    - 单词个数: `mutate(word_count = str_count(text, "\\S+"))`
- **添加新列后记得 assign 回原变量, 以便后续继续使用**
    - `data <- data |> mutate(new_col = ...)`
- **利用 RStudio Snippet 提高效率 [(详细介绍)](#rstudio-snippet)**
